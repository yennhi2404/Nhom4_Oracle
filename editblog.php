...
<div class="site-wrap">
    ...
    <section class="site-section">
      <div class="container">
        ...
        if($flag==true){  
$idblog = $_REQUEST["id"];
    $sql = oci_parse($conn,"SELECT ID, PAGE_TITLE, PAGE_CONTENT,CATEGORY, POST_BY_AD_ID FROM CMS_CONTENT c WHERE ID = '$idblog'"); //Lấy thông tin blog có mã id tương ứng
    oci_execute($sql);
      while($row = oci_fetch_array($sql)){
        ...
        if(isset($_POST["Luu"])) { //Khi nhấn Lưu sẽ thực hiện các câu lệnh            
            ...
            $idtacgia = test_input($_POST["idtacgia"]);
            $hangmuc= test_input($_POST["hangmuc"]);
            ...
            $result = oci_parse($conn,"UPDATE CMS_CONTENT SET PAGE_TITLE = '$tieude', PAGE_CONTENT='$noidung',UPDATED_AT=sysdate, IMAGES = '$stringhinh', POST_BY_AD_ID = '$idtacgia',CATEGORY='$hangmuc' WHERE ID = '$idblog'"); //Cập nhật dữ liệu tương ứng vào database
 ...
 ?>
    ...
    <table align="center">
    <tr>
        <td colspan="2"><h2 style="text-align: center; font-family: Arial">Sửa đổi blog</h2></td>
    </tr>    
    <tr>
        <td>Tiêu đề: </td>
        <td><label>
            <input type="text" name="tieude" id="tieude" style="width: 300px" value="<?php if(isset($_POST["tieude"])) echo $_POST["tieude"]; else echo $tieude; ?>">
        </label></td>
    </tr>
    <tr>
        <td>Nội dung: </td>
        <td><label>
            <textarea type="text" name="noidung" id="noidung" style="width: 300px"><?php if(isset($_POST["noidung"])) echo $_POST["noidung"]; else echo $noidung; ?></textarea>
        </label></td>
    </tr>
    <tr>
        <td>Hình ảnh: </td>
    ...
    <tr>
        <td>ID tác giả: </td>
        <td><label>
            <input type="text" name="idtacgia" id="idtacgia" value="<?php if(isset($_POST["idtacgia"])) echo $_POST["idtacgia"]; else echo $idtacgia; ?>">
        </label></td>
    </tr>
    <tr>
        <td>Hạng mục: </td>
        <td><label>
            <input type="text" name="hangmuc" id="hangmuc" value="<?php if(isset($_POST["hangmuc"])) echo $_POST["hangmuc"]; else echo $hangmuc;?>">
        </label></td>
    </tr>
    <tr>
        <td>Hình ảnh phụ: </td>
    ...
    <tr>
        <td colspan="2"><a href="quanlyblog.php">Quay về</a></td>
    </tr>
</table>
</form>
<?php } ?>
