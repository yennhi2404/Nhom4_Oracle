...
// Tìm Start
            ...
            $que="select * from (
                                  select 
                                   ID,TITLE,NAMEIMG,LOCATION,JOB_TYPE_ID,COMPANY_ID,
                                   row_number() over
                                  (order by ID DESC) rn
                                      from JOBS)
            ...
            $id=$row[0];
                $comp_id=$row[5];
              if($comp_id!=0){
              $queryct="SELECT NAME FROM COMPANIES WHERE ID = $comp_id";
            $kqq=oci_parse($conn, $queryct);
            oci_execute($kqq);
            while($dong=oci_fetch_row($kqq)){
               $namect=$dong[0];}
           }
            ...
            echo   '<div class="job-listing-position custom-width w-50 mb-3 mb-sm-0">';
             echo'   <h2>'.$row[1].'</h2>';
             echo '<span class="ml-0 mr-2 mb-2"><span class="icon-briefcase mr-2"></span>'.$namect.'</span>';
              echo '</div>';
        ...
        <div class="row pagination-wrap mt-5">
          <div class="col-md-6 text-center text-md-left mb-4 mb-md-0">
            <span>Hiển thị <?php echo oci_num_rows($result); ?> công việc</span>
          </div>
          <div class="col-md-12 text-center ">
           ...
           if ($current_page > 1 && $total_page > 1){
                echo '<a href="phantrang.php?page='.($current_page-1).'" class ="prev">Trước</a>';
            }
          echo '<div class="d-inline-block">';
            ...
            if ($current_page < $total_page && $total_page > 1){
                echo '<a href="phantrang.php?page='.($current_page+1).'" class="next">Tiếp</a>';
            }
