...
<div class="site-wrap">
    ...
    <section class="site-section">
      ...
      if($flag==true){     
        ...
        $sql = oci_parse($conn,"SELECT FULL_NAME,EMAIL,PASSWORD,ID_LOAI_USER FROM USERS WHERE ID = '$idtk'"); //Lấy thông tin dự án có mã id tương ứng
    oci_execute($sql);
      while($row = oci_fetch_array($sql)){
        ...
        if(isset($_POST["Luu"])){  //Khi nhấn Kiểm duyệt sẽ thực hiện các lệnh
          ...
          $loai = test_input($_POST["loai"]); 

          $hashmatkhau = md5($matkhau); 

          $check = 1;
          if($ten==''||$email==''||$matkhau==''||$loai==''){ //Kiểm tra rỗng nếu không nhập (hoặc thiếu)
                echo "<p align='center'>Vui lòng nhập đủ thông tin!</p>";
                $check = 0;
          }

          if($_POST["matkhau"] == $matkhauu){
            $result = oci_parse($conn,"UPDATE USERS SET FULL_NAME = '$ten', EMAIL = '$email', ID_LOAI_USER = '$loai', UPDATED_AT = sysdate WHERE ID = '$idtk'"); //Cập nhật dữ liệu tương ứng vào database
          }else{
            $result = oci_parse($conn,"UPDATE USERS SET FULL_NAME = '$ten', EMAIL = '$email',PASSWORD = '$hashmatkhau', ID_LOAI_USER = '$loai', UPDATED_AT = sysdate WHERE ID = '$idtk'"); //Cập nhật dữ liệu tương ứng vào database
          }
 ...
 ?>
        ...
        <table align="center">
        <tr>
            <td colspan="3"><h2 style="text-align: center; font-family: Arial">Sửa đổi tài khoản</h2></td>
        </tr>
        <tr>
        <td>Họ tên: </td>
        <td><label>
    ...
    <tr>
        <td>Mật khẩu: </td>
        <td><label>
            <input type="text" name="matkhau" id="matkhau" style="width: 700px" value="<?php if(isset($_POST["matkhau"])) echo $_POST["matkhau"]; else echo $matkhauu; ?>">
        </label></td>
    </tr>
    <tr>
        <td>Loại tài khoản: </td>
        <td><label>
            <input type="text" name="loai" id="loai" style="width: 700px" value="<?php if(isset($_POST["loai"])) echo $_POST["loai"]; else echo $loaii; ?>">
        </label></td>
        <td>(Admin: 1, User: 2) </td>
    </tr>
    <br>
    <tr>
    ...
    <tr>
        <td colspan="2"><a href="quanlyuser.php?page=1">Quay về</a></td>
    </tr>
    </table>
    </form>
<?php } ?>
