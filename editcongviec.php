...
<div class="site-wrap">
    ...
    <section class="site-section">
      ...
      if($flag==true){       
        ...
        $sql = oci_parse($conn,"SELECT JOBS.TITLE,JOB_TYPE,HIDE_SALARY,c.WEBSITE,c.NAME,JOBS.EMAIL FROM JOBS JOIN COMPANIES c ON JOBS.COMPANY_ID = c.ID JOIN JOB_TYPES ON JOBS.JOB_TYPE_ID=JOB_TYPES.ID WHERE JOBS.ID = '$idcv'"); //Lấy thông tin công việc có mã id tương ứng
      oci_execute($sql);
      while($row = oci_fetch_array($sql)){
        ...
        if(isset($_POST["Luu"])){  //Khi nhấn Kiểm duyệt sẽ thực hiện các lệnh
          $tenviec = test_input($_POST["tenviec"]);
          $loaiviec = test_input($_POST["loaiviec"]); 
          $luong = test_input($_POST["luong"]);
          $website = test_input($_POST["website"]); 
          $tenct = test_input($_POST["tenct"]); 
          $email = test_input($_POST["email"]);  

          $check = 1;
          if($tenviec==''||$loaiviec==''||$luong==''||$website==''||$tenct==''||$email==''){ //Kiểm tra rỗng nếu không nhập (hoặc thiếu)
                echo "<p align='center'>Vui lòng nhập đủ thông tin!</p>";
                $check = 0;
          }

          $queryct="SELECT ID FROM COMPANIES WHERE NAME='$tenct'";
          $kqct=oci_parse($conn, $queryct);
          oci_execute($kqct);
          while($row=oci_fetch_row($kqct)){
            $compa_id=$row[0];
          }
          $resultct = oci_parse($conn,"UPDATE COMPANIES SET WEBSITE = '$website',NAME = '$tenct', UPDATED_AT = sysdate WHERE ID = '$compa_id'"); //Cập nhật dữ liệu tương ứng vào database
          

          $queryloai="SELECT ID FROM JOB_TYPES WHERE JOB_TYPE='$loaiviec'";
          $kqloai=oci_parse($conn, $queryloai);
          oci_execute($kqloai);
          while($row=oci_fetch_row($kqloai)){
            $loai_id=$row[0];
          }
          echo $loai_id;
          $resultv = oci_parse($conn,"UPDATE JOBS SET TITLE = '$tenviec',JOB_TYPE_ID = $loai_id, HIDE_SALARY = '$luong',EMAIL = '$email', UPDATED_AT = sysdate WHERE ID = '$idcv'"); //Cập nhật dữ liệu tương ứng vào database
          
          if($check<>0){ //Kiểm tra và xuất thông báo
            oci_execute($resultct);
            oci_execute($resultv);
 ...
 ?>
  ...
  <form name="form1" method="post" action=""  enctype="multipart/form-data">
        <table align="center">
        <tr>
            <td colspan="3"><h2 style="text-align: center; font-family: Arial">Sửa đổi công việc</h2></td>
        </tr>
    <tr>
        <td>Tên việc: </td>
        <td><label>
            <input type="text" name="tenviec" id="tenviec" style="width: 700px" value="<?php if(isset($_POST["tenviec"])) echo $_POST["tenviec"]; else echo $tenviecc; ?>">
        </label></td>
    </tr>
    <tr>
        <td>Loại việc: </td>
        <td><label>
            <input type="text" name="loaiviec" id="loaiviec" style="width: 700px" value="<?php if(isset($_POST["loaiviec"])) echo $_POST["loaiviec"]; else echo $loaiviecc; ?>">
        </label></td>
    </tr>
    <tr>
        <td>Lương: </td>
        <td><label>
            <input type="text" name="luong" id="luong" style="width: 700px" value="<?php if(isset($_POST["luong"])) echo $_POST["luong"]; else echo $luongg; ?>">
        </label></td>
    </tr>
    <tr>
        <td>Website: </td>
        <td><label>
            <input type="text" name="website" id="website" style="width: 700px" value="<?php if(isset($_POST["website"])) echo $_POST["website"]; else echo $websitee; ?>">
        </label></td>
    </tr>
    <tr>
        <td>Tên công ty: </td>
        <td><label>
            <input type="text" name="tenct" id="tenct" style="width: 700px" value="<?php if(isset($_POST["tenct"])) echo $_POST["tenct"]; else echo $tenctt; ?>">
        </label></td>
    </tr>
    <tr>
        <td>Email: </td>
    ...
    <tr>
        <td colspan="2"><a href="quanlycongviec.php?page=1">Quay về</a></td>
    </tr>
    </table>
    </form>
<?php } ?>
