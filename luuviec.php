...
<div class="site-wrap">
 ...
 //Lay id user
    ...
    //Kiểm tra đã tồn tại chưa
            $kt="SELECT * FROM JOB_APPLY WHERE USER_ID = '$idus' AND JOB_ID= '$id_job'";
            $run=oci_parse($conn,$kt);
            oci_execute($run);
            while(oci_fetch_array($run)){} 
            if(oci_num_rows($run)<1){ //Nếu chưa thì insert
              //Insert into JOB_APPLY
              $query="INSERT INTO JOB_APPLY(USER_ID,JOB_ID,CREATED_AT) VALUES($idus,'$id_job',sysdate)";
    ...
    //Hien thi ra man hinh
            ...
            while($row=oci_fetch_row($comp)){
                echo '<ul class="job-listings mb-5">';
            	  $job_id=$row[0];
            	  if($job_id!=""){
                  $sel="SELECT JOBS.ID,JOBS.TITLE,JOBS.NAMEIMG,JOBS.LOCATION,JOB_TYPE,COMPANIES.NAME FROM JOBS JOIN JOB_TYPES ON JOBS.JOB_TYPE_ID = JOB_TYPES.ID JOIN COMPANIES ON JOBS.COMPANY_ID = COMPANIES.ID WHERE JOBS.ID='$job_id'";
                  ...
                  while($row=oci_fetch_row($ex)){
                    ...
                    echo '<img src=images/'.$row[2].' class="img-fluid">';
                    echo '</div>';
                    ...
                    echo   '<div class="job-listing-position custom-width w-50 mb-3 mb-sm-0">';
                     echo'   <h2>'.$row[1].'</h2>';
                     echo '<span class="ml-0 mr-2 mb-2"><span class="icon-briefcase mr-2"></span>'.$row[5].'</span>';
                      echo '</div>';
                     ...
                     echo '<span class="icon-room">'.$row[3].'</span>';
                      ...
                      //  echo '<li>' . $row[0] . '</li>';
                       echo '<span class="badge badge-danger">'.$row[4].'</span>';
                     echo ' </div>';
                    echo '</div>';
