...
<div class="site-wrap">
    ...
    <!-- HOME -->
     <section class="section-hero overlay inner-page bg-image" style="background-image: url('images/hero_1.jpg');" id="home-section">
      <div class="container">
        <div class="row">
          <div class="col-md-7">
            <h1 class="text-white font-weight-bold">Hỏi đáp</h1>
            <div class="custom-breadcrumbs">
              <a href="admin.php">Trang chủ</a> <span class="mx-2 slash">/</span>
              <span class="text-white"><strong>Hỏi đáp</strong></span>
            </div>
          </div>
        </div>
      </div>
    ...
    <section class="site-section">
      <div class="container">
        ...
        if($flag==true){     
        echo '<h3 align="center" class="style1" style="font-weight:bold">CÂU HỎI THƯỜNG GẶP</h3>
                <p align="center">
                    ...
                    <table>
                        <tr style="color:red; font-weight: bold">
                            <th width="90">ID </th>
                            <th width="1900">Câu hỏi </th>
                            <th width="1900">Trả lời </th>
                            <th width="90">Số lần hỏi </th>
                            <th width="170">Ngày đăng</th>
                            <th width="170">Ngày cập nhật</th>
                            <th width="100">Sửa </th>
                            <th width="100">Xóa </th>
                        </tr>';
            if(isset($_GET['page'])!=""){
            ...
            $result = oci_parse($conn,"SELECT ID,FAQ_QUESTION,FAQ_ANSWER,SORT_ORDER,TO_CHAR(CREATED_AT,'DD-MM-YYYY HH24:MI:SS'), TO_CHAR(UPDATED_AT,'DD-MM-YYYY HH24:MI:SS') FROM FAQS ORDER BY SORT_ORDER DESC");
            oci_execute($result);
               ...
               if($i>=$startitem){
                 ...
                 $dap = $row[2]->load();
                      echo "<tr>
                            ...
                            <td>$row[4]</td>
                            <td>$row[5]</td>
                            <td><a href='editcauhoi.php?id=".$row[0]."'>Sửa</a></td>
                            <td><a onClick=\"javascript: return confirm('Bạn có đồng ý xóa hỏi đáp có mã $row[0]?');\" href='quanlycauhoi.php?id=".$row[0]."'>Xóa</a></td>
                            </tr>";                   
                       ++$dem;
                      if($dem==6)                           
                        break;
            ...
            echo '</tr>
                            <tr>
                                <td colspan="11">
                                    <a href="themcauhoi.php"><input type="submit" name="themsp" value="Thêm hỏi đáp"></a>
                                </td>
                            </tr>
                        </table>';
     ...
     if(isset($_GET["id"])!=''){      
        $idch = $_REQUEST["id"];   
        $result = oci_parse($conn,"DELETE FROM FAQS where ID = $idch");
        oci_execute($result);
        if(oci_execute($result)){ //Kiểm tra và xuất thông báo
            echo "<p align = 'center'>Đã xóa hỏi đáp có mã <b>$idch</b></p>";            
        }
        else echo oci_error($conn); // Nếu không tìm thấy dịch vụ thì xuất ra kết nối thất bại
    }
            oci_close($conn);
     ?>
<div class="row pagination-wrap mt-5">
          <?php 
            include "dbconnect.php";
            $result = oci_parse($conn,"SELECT * FROM FAQS");
            oci_execute($result);
            ...
            <div class="custom-pagination ml-auto">
              <?php 
                if(isset($_GET["page"]) !=''){
                  ...
                  $lui=$get-1;
                  echo '<a href="quanlycauhoi.php?page='.$lui.'" class="prev">Trước</a>';
              ?>
              ...
              <?php 
                                ...
                                if($sl_sp>6){
                                    $npage = ceil($sl_sp/6);
                                    for($i = 1;$i<=$npage;++$i){
                                        echo '<a href="quanlycauhoi.php?page='.$i.'">'.$i.'</a>';    
                                    }
                                    
                                }
                                else echo '<a href="quanlycauhoi.php?page=1">1</a>'; 
                             ?>
              </div>
              <?php 
                if(isset($_GET["page"])!=''){
                ...
                $tiep=$get+1;
                echo '<a href="quanlycauhoi.php?page='.$tiep.'" class="next">Tiếp</a>';
              ?>
            </div>
            <br>
