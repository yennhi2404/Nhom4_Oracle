...
<div class="site-wrap">
    ...
    <table align="center">
    <tr>
        <td colspan="2"><h2 style="text-align: center; font-family: Arial">Thêm dịch vụ mới</h2></td>
    </tr>    
    <tr>
        <td>Tiêu đề: </td>
        <td><label>
    ...
    <tr>
        <td colspan="2"><a href="quanlydichvu.php?page=1">Quay về</a></td>
    </tr>
</table>
</form>
<?php
include "dbconnect.php";
        ...
        if(isset($_POST["Luu"])) { //Khi nhấn Lưu sẽ thực hiện các câu lệnh            
            ...
            $check = 1;
            if($tieude==''||$noidung==''){ //Kiểm tra rỗng nếu không nhập (hoặc thiếu)
                echo "<p align='center'>Vui lòng nhập đủ thông tin!</p>";
                $check = 0;
            }
            
            $result=oci_parse($conn,"INSERT INTO CMS (ID,SEO_TITLE,SEO_DESCRIPTION,CREATED_AT) VALUES ('','$tieude','$noidung',sysdate)"); //Thêm dữ liệu tương ứng vào database 

            if($check<>0){ //Kiểm tra và xuất thông báo
                oci_execute($result);
                echo "<br><h3 align='center' style='color:red'>Thêm dịch vụ mới thành công!</h3>";
            }else echo "<h3 align='center' style='color:red'>Thêm dịch vụ mới thất bại! </h3>".oci_error($result);                
        }  
    }  
      oci_close($conn);
 ?>
