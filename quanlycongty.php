    ...
    <!-- HOME -->
     <section class="section-hero overlay inner-page bg-image" style="background-image: url('images/hero_1.jpg');" id="home-section">
      <div class="container">
        <div class="row">
          <div class="col-md-7">
            <h1 class="text-white font-weight-bold">Công ty</h1>
    ...
    <section class="site-section">
      <div class="container">
        <?php           
          ...
          if($flag==true){  
        echo '<h3 align="center" class="style1" style="font-weight:bold">CÔNG TY</h3>
                    <table>
                        <tr style="color:red; font-weight: bold">
                            <th width="170">ID </th>
                            <th width="900">Tên công ty </th>
            ...
            $result = oci_parse($conn,"SELECT ID,NAME,WEBSITE,TO_CHAR(CREATED_AT,'DD-MM-YYYY HH24:MI:SS'), TO_CHAR(UPDATED_AT,'DD-MM-YYYY HH24:MI:SS') FROM COMPANIES");
            oci_execute($result);
            ...
            while($row = oci_fetch_array($result)){
              if($i>=$startitem){
                      echo "<tr>
                            ...
                            <td>$row[3]</td>
                            <td>$row[4]</td>
                            <td><a href='editcongty.php?id=".$row[0]."'>Sửa</a></td>
                            <td><a onClick=\"javascript: return confirm('Bạn có đồng ý xóa công ty có mã $row[0]?');\" href='quanlycongty.php?id=".$row[0]."'>Xóa</a></td>
                            </tr>";                   
                       ++$dem;
                      if($dem==12)                           
                        break;
     ...
     if(isset($_GET["id"])!=''){      
        $idct = $_REQUEST["id"];   
        $result = oci_parse($conn,"DELETE FROM COMPANIES where ID = $idct");
        oci_execute($result);
        if(oci_execute($result)){ //Kiểm tra và xuất thông báo
            echo "<p align = 'center'>Đã xóa công ty có mã <b>$idct</b></p>";            
        }
        else echo "Không thể xóa công ty này!"; 
    }
            oci_close($conn);
     ?>
<div class="row pagination-wrap mt-5">
          <?php 
            include "dbconnect.php";
            $result = oci_parse($conn,"SELECT * FROM COMPANIES");
            oci_execute($result);
            ...
            <div class="custom-pagination ml-auto">
              <?php 
                if(isset($_GET["page"]) !=''){
                  ...
                  $lui=$get-1;
                  echo '<a href="quanlycongty.php?page='.$lui.'" class="prev">Trước</a>';
              ?>
              ...
              <?php 
                                ...
                                if($sl_sp>12){
                                    $npage = ceil($sl_sp/12);
                                    for($i = 1;$i<=$npage;++$i){
                                        echo '<a href="quanlycongty.php?page='.$i.'">'.$i.'</a>';    
                                    }
                                    
                                }
                                else echo '<a href="quanlycongty.php?page=1">1</a>'; 
                             ?>
              </div>
              <?php 
                if(isset($_GET["page"])!=''){
                ...
                $tiep=$get+1;
                echo '<a href="quanlycongty.php?page='.$tiep.'" class="next">Tiếp</a>';
              ?>
            </div>
            <br>
