...
<div class="site-wrap">
    ...
    <section class="site-section">
      <div class="container">
        ...
        if($flag==true){  
        $idch = $_REQUEST["id"];
        $sql = oci_parse($conn,"SELECT FAQ_QUESTION,FAQ_ANSWER,SORT_ORDER FROM FAQS WHERE ID = '$idch'"); //Lấy thông tin dự án có mã id tương ứng
    oci_execute($sql);
      while($row = oci_fetch_array($sql)){
        ...
        if(isset($_POST["Luu"])){  //Khi nhấn Kiểm duyệt sẽ thực hiện các lệnh
          $hoi = test_input($_POST["hoi"]);
          $dap = test_input($_POST["dap"]); 
          $solan = test_input($_POST["solan"]); 

          $check = 1;
          if($hoi==''||$dap==''||$solan==''){ //Kiểm tra rỗng nếu không nhập (hoặc thiếu)
                echo "<p align='center'>Vui lòng nhập đủ thông tin!</p>";
                $check = 0;
          }

          $result = oci_parse($conn,"UPDATE FAQS SET FAQ_QUESTION = '$hoi', FAQ_ANSWER='$dap', SORT_ORDER='$solan', UPDATED_AT = sysdate WHERE ID = '$idch'"); //Cập nhật dữ liệu tương ứng vào database
 ...
 ?>
        ...
        <table align="center">
        <tr>
            <td colspan="2"><h2 style="text-align: center; font-family: Arial">Sửa đổi câu hỏi đáp</h2></td>
        </tr>
        <tr>
        <td>Câu hỏi: </td>
        <td><label>
            <textarea type="text" name="hoi" id="hoi" style="width: 300px"><?php if(isset($_POST["hoi"])) echo $_POST["hoi"]; else echo $hoi; ?></textarea>
        </label></td>
    </tr>
    <tr>
        <td>Trả lời: </td>
        <td><label>
            <textarea type="text" name="dap" id="dap" style="width: 300px"><?php if(isset($_POST["dap"])) echo $_POST["dap"]; else echo $dap; ?></textarea>
        </label></td>
    </tr> 
    <tr>
        <td>Số lần hỏi: </td>
        <td><label>
            <input type="text" name="solan" id="solan" style="width: 300px" value="<?php if(isset($_POST["solan"])) echo $_POST["solan"]; else echo $solan; ?>">
        </label></td>
    </tr>
    <br>
    <tr>
    ...
    <tr>
        <td colspan="2"><a href="quanlycauhoi.php?page=1">Quay về</a></td>
    </tr>
    </table>
    </form>
<?php } ?>
