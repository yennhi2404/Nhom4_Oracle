...
<div class="site-wrap">
    ...
    <table align="center">
    <tr>
        <td colspan="2"><h2 style="text-align: center; font-family: Arial">Thêm quốc gia mới</h2></td>
    </tr> 
    <tr>
        <td>Tên quốc gia: </td>
        <td><label>
            <input type="text" name="tenqg" id="tenqg" style="width: 300px" value="<?php if(isset($_POST['tenqg'])) echo $_POST['tenqg']; else echo '' ?>" >
        </label></td>
    </tr>    
    <br>
      <tr>
...
<?php
        if(isset($_POST["Tao"])) { //Khi nhấn Lưu sẽ thực hiện các câu lệnh     
            ...
            $result=oci_parse($conn,"INSERT INTO COUNTRIES (COUNTRY) VALUES ('$tenqg')"); //Thêm dữ liệu tương ứng vào database 

            if($check<>0){ //Kiểm tra và xuất thông báo
                oci_execute($result);
                echo "<br><h3 align='center' style='color:red'>Thêm quốc gia mới thành công!</h3>";
            }else echo "<h3 align='center' style='color:red'>Thêm quốc gia mới thất bại! </h3>".oci_error($result);                
        }} 
      oci_close($conn);
 ?>
