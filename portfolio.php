...
<div class="site-wrap">
    ...
    <section class="section-hero overlay inner-page bg-image" style="background-image: url('images/hero_1.jpg');" id="home-section">
      <div class="container">
        <div class="row">
          <div class="col-md-7">
            <h1 class="text-white font-weight-bold">Danh mục đầu tư (Trang bổ sung)</h1>
            <div class="custom-breadcrumbs">
              <a href="index.php">Trang chủ</a> <span class="mx-2 slash">/</span>
              <span class="text-white"><strong>Danh mục đầu tư </strong></span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="container">
        <div class="row justify-content-center mb-5" data-aos="fade-up">
          <div id="filters" class="filters text-center button-group col-md-7">
            ...
            <!-- <button class="btn btn-primary" data-filter=".web">W</button>
            <button class="btn btn-primary" data-filter=".web">Design</button>
            <button class="btn btn-primary" data-filter=".web">Brand</button>
          </div>
        </div>  --> 
            <?php 
              $sql="SELECT NAME FROM PROFILE_PROJECTS ORDER BY ID DESC";
              $run=oci_parse($conn,$sql);
              oci_execute($run);
              $i=0;
              while ($row=oci_fetch_array($run)){
                if($i==0)
                  echo '<button class="btn btn-primary" data-filter=".web">'.$row[0].'</button>';
                if($i==1)
                  echo '<button class="btn btn-primary" data-filter=".design">'.$row[0].'</button>';
                if($i==2){
                  echo '<button class="btn btn-primary" data-filter=".brand">'.$row[0].'</button>';
                  $i=-1;
                }
                $i++;
              }
            ...
            <?php
              $orc="SELECT IMAGE,ID FROM PROFILE_PROJECTS ORDER BY ID DESC";
              $run=oci_parse($conn,$orc);
              oci_execute($run);
              $i=0;
              while ($row=oci_fetch_array($run)){
                $gallery = explode('*',$row[0],4); 
                if($i==0){                     
                      echo '<div class="item web col-6 col-sm-6 col-md-6 col-lg-4 col-xl-4 mb-4">
                              <a href="portfolio-single.php?iddd='.$row[1].'"class="item-wrap">
                                <span class="icon-add"></span>
                                  <img class="img-fluid" src="images/'.$gallery[0].'"></a></div>';
                      echo '<div class="item web col-6 col-sm-6 col-md-6 col-lg-4 col-xl-4 mb-4">
                              <a href="portfolio-single.php?iddd='.$row[1].'"class="item-wrap">
                                <span class="icon-add"></span>
                                  <img class="img-fluid" src="images/'.$gallery[1].'"></a></div>';
                      echo '<div class="item web col-6 col-sm-6 col-md-6 col-lg-4 col-xl-4 mb-4">
                              <a href="portfolio-single.php?iddd='.$row[1].'"class="item-wrap">
                                <span class="icon-add"></span>
                                  <img class="img-fluid" src="images/'.$gallery[2].'"></a></div>';
                }
                if($i==1){
                      echo '<div class="item design col-6 col-sm-6 col-md-6 col-lg-4 col-xl-4 mb-4">
                              <a href="portfolio-single.php?iddd='.$row[1].'"class="item-wrap">
                                <span class="icon-add"></span>
                                  <img class="img-fluid" src="images/'.$gallery[0].'"></a></div>';
                      echo '<div class="item design col-6 col-sm-6 col-md-6 col-lg-4 col-xl-4 mb-4">
                ...
                if($i==2){
                      echo '<div class="item brand col-6 col-sm-6 col-md-6 col-lg-4 col-xl-4 mb-4">
                              <a href="portfolio-single.php?iddd='.$row[1].'"class="item-wrap">
                                <span class="icon-add"></span>
                                  <img class="img-fluid" src="images/'.$gallery[0].'"></a></div>';
                      echo '<div class="item brand col-6 col-sm-6 col-md-6 col-lg-4 col-xl-4 mb-4">
