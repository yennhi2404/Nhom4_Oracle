...
$idthu = $_REQUEST["id"];
        ...
        if(isset($_POST["duyet"])){  //Khi nhấn Kiểm duyệt sẽ thực hiện các lệnh
          $trangthai = test_input($_POST["tt"]); 
          $id_thu = test_input($_POST["idthu"]);  
          $result = oci_parse($conn,"UPDATE CONTACT_MESSAGES SET KIEM_DUYET = '$trangthai', UPDATED_AT=sysdate  WHERE ID LIKE '$idthu'"); //Cập nhật dữ liệu tương ứng vào database
          oci_execute($result);
          echo "<p align = 'center'>Cập nhật thành công!!!!</p>";
        }
        echo "<p align = 'center'><a href = 'quanlythu.php?page=1'>Quay về hộp thư</a></p>";
      oci_close($conn);
 ?>
        ...
        <table align="center">
        <tr>
            <td colspan="2"><h2 style="text-align: center; font-family: Arial">Kiểm duyệt thư</h2></td>
        </tr>
        <tr>
            <td>Mã thư: <input type="text" name="idthu" id="idthu" value="<?php if(isset($_POST["idthu"])) echo $_POST["idthu"]; else echo $idthu; ?>" readonly=true></td>
            <td>Trạng thái: <input type="text" name="tt" id="tt" value="<?php if(isset($_POST["tt"])) echo $_POST["tt"]; else echo ''; ?>"></td>
        </tr>
        <tr>
            <td colspan="2" align="center"><input type="submit" name="duyet" value="Duyệt"></td>
        </tr>
        </table>
    </form>
<?php } ?>
