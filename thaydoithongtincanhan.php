    ...
    <section class="site-section">
      <div class="container">
        ...
        function getExtension($str) { // Hàm này đọc phần mở rộng của file. Nó được dùng để kiểm tra nếu file này có phải là file hình hay không 
            ...
            $sql = oci_parse($conn,"SELECT FULL_NAME,NAME,DATE_OF_BIRTH,GENDER_ID,EMAIL,PASSWORD,PHONE, IMAGE FROM USERS WHERE NAME = '$cur_user'"); //Lấy thông tin dự án có mã id tương ứng
            oci_execute($sql);
        ...
        if(isset($_POST["Luu"])){  //Khi nhấn Kiểm duyệt sẽ thực hiện các lệnh
          ...
          $check = 1;
          if($ten==''||$tendn==''||$email==''||$matkhau==''){ //Kiểm tra rỗng nếu không nhập (hoặc thiếu)
                echo "<p align='center'>Vui lòng nhập đầy đủ các mục (*)!</p>";
                $check = 0;
          }
          $uploadOk = 1;
          if ($hinh <> ''){ 
                //Lấy phần mở rộng của file
                $extension = getExtension($hinh);
          ...
          if($_POST["matkhau"] == $matkhauu){
            $result = oci_parse($conn,"UPDATE USERS SET FULL_NAME = '$ten', DATE_OF_BIRTH = '$ngaysinh', GENDER_ID = '$gioitinh', EMAIL = '$email', PHONE = '$sdt', IMAGE = '$hinh', UPDATED_AT = sysdate WHERE NAME = '$tendn'"); //Cập nhật dữ liệu tương ứng vào database
          }else{
            $result = oci_parse($conn,"UPDATE USERS SET FULL_NAME = '$ten', DATE_OF_BIRTH = '$ngaysinh', GENDER_ID = '$gioitinh', EMAIL = '$email', PHONE = '$sdt', IMAGE = '$hinh', PASSWORD = '$hashmatkhau', UPDATED_AT = sysdate WHERE NAME = '$tendn'"); //Cập nhật dữ liệu tương ứng vào database
          }
          if($check<>0){ //Kiểm tra và xuất thông báo
            oci_execute($result);
            echo "<p align = 'center'>Cập nhật thành công!!!!</p>";
          }else echo "<p align = 'center'>Cập nhật thất bại!!!!</p>";
        }}}
      oci_close($conn);
 ?>

      <form name="form1" method="post" action=""  enctype="multipart/form-data" onsubmit="return ValidateEmail(document.form1.email)">
        <table align="center">
        <tr>
            <td colspan="2"><h2 style="text-align: center; font-family: Arial">THÔNG TIN CÁ NHÂN</h2></td>
        </tr>
        <tr>
          <td>Họ tên: (*)</td>
          <td><label>
              <input type="text" name="ten" id="ten" style="width: 700px" value="<?php if(isset($_POST["ten"])) echo $_POST["ten"]; else echo $tenn; ?>">
          </label></td>
        </tr>
        <tr>
          <td>Username: </td>
          <td><label>
              <input type="text" name="tendn" id="tendn" readonly style="width: 700px" value="<?php if(isset($_POST["tendn"])) echo $_POST["tendn"]; else echo $tendnn; ?>">
          </label></td>
        </tr>
        <tr>
          <td>Ngày sinh: </td>
          <td><label>
              <input type="text" name="ngaysinh" id="ngaysinh" style="width: 700px" value="<?php if(isset($_POST["ngaysinh"])) echo $_POST["ngaysinh"]; else echo $ngaysinhh; ?>">
          </label></td>
          <td>(VD: 08-MAR-89)</td>
        </tr>
        <tr>
          <td>Giới tính: </td>
          <td><label>
              <input type="text" name="gioitinh" id="gioitinh" style="width: 700px" value="<?php if(isset($_POST["gioitinh"])) echo $_POST["gioitinh"]; else echo $gioitinhh; ?>">
          </label></td>
          <td>(1: Nữ, 2: Nam, 3: Khác)</td>
        </tr>

    <tr>
        <td>Email: (*)</td>
        <td><label>
            <input type="text" name="email" id="email" style="width: 700px" value="<?php if(isset($_POST["email"])) echo $_POST["email"]; else echo $emaill; ?>">
        </label></td>
    </tr> 
    <tr>
        <td>Mật khẩu: (*)</td>
        <td><label>
            <input type="password" name="matkhau" id="matkhau" style="width: 700px" value="<?php if(isset($_POST["matkhau"])) echo $_POST["matkhau"]; else echo $matkhauu; ?>">
        </label></td>
    </tr>
    <tr>
        <td>Số điện thoại: </td>
        <td><label>
            <input type="text" name="sdt" id="sdt" style="width: 700px" value="<?php if(isset($_POST["sdt"])) echo $_POST["sdt"]; else echo $sdtt; ?>">
        </label></td>
    </tr>
    <tr>
        <td>Hình đại diện: </td>
        ...
        <?php
            echo '<td><img src="images/'.$hinhh.'" alt="Avatar"></td>';
        ?>
    </tr>
    <br>
    <tr>
    ...
    <tr>
        <td colspan="2"><a href="index.php">Quay về</a></td>
    </tr>
    </table>
    </form>
