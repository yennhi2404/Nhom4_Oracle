...
<div class="site-wrap">
    ...
    <!-- HOME -->
     <section class="section-hero overlay inner-page bg-image" style="background-image: url('images/hero_1.jpg');" id="home-section">
      <div class="container">
        <div class="row">
          <div class="col-md-7">
            <h1 class="text-white font-weight-bold">Quốc gia</h1>
            <div class="custom-breadcrumbs">
              <a href="index.php">Trang chủ</a> <span class="mx-2 slash">/</span>
              <span class="text-white"><strong>Quốc gia</strong></span>
            </div>
          </div>
        </div>
      </div>
    ...
    <section class="site-section">
      <div class="container">
        ...
        if($flag==true){     
        $idqg = $_REQUEST["idqg"];
        $sql = oci_parse($conn,"SELECT COUNTRY FROM COUNTRIES WHERE ID = '$idqg'"); //Lấy thông tin dự án có mã id tương ứng
    oci_execute($sql);
      while($row = oci_fetch_array($sql))
        ...
        if(isset($_POST["Luu"])){  //Khi nhấn Kiểm duyệt sẽ thực hiện các lệnh
          $tenqg = test_input($_POST["tenqg"]);
          
          $check = 1;
          if($tenqg==''){ //Kiểm tra rỗng nếu không nhập (hoặc thiếu)
                echo "<p align='center'>Vui lòng nhập đủ thông tin!</p>";
                $check = 0;
          }

          $result = oci_parse($conn,"UPDATE COUNTRIES SET COUNTRY = '$tenqg' WHERE ID = '$idqg'"); //Cập nhật dữ liệu tương ứng vào database
 ...
 ?>
        ...
        <table align="center">
        <tr>
            <td colspan="2"><h2 style="text-align: center; font-family: Arial">Sửa đổi quốc gia</h2></td>
        </tr>
        <tr>
          <td>Tên quốc gia: </td>
          <td><label>
              <input type="text" name="tenqg" id="tenqg" style="width: 300px" value="<?php if(isset($_POST["tenqg"])) echo $_POST["tenqg"]; else echo $tenqgg; ?>">
          </label></td>
        </tr>
