...
<div class="site-wrap">
    ...
    <!-- HOME -->
     <section class="section-hero overlay inner-page bg-image" style="background-image: url('images/hero_1.jpg');" id="home-section">
      <div class="container">
        <div class="row">
          <div class="col-md-7">
            <h1 class="text-white font-weight-bold">Hộp thư</h1>
            <div class="custom-breadcrumbs">
              <a href="index.php">Trang chủ</a> <span class="mx-2 slash">/</span>
              <span class="text-white"><strong>Hộp thư</strong></span>
            </div>
          </div>
        </div>
      </div>
    ...
    <section class="site-section">
      <div class="container">
        ...
        if($flag==true){   
        echo '<h3 align="center" class="style1" style="font-weight:bold">THƯ CỦA KHÁCH HÀNG</h3>
                <p align="center">
                    ...
                    <table>
                        <tr style="color:red; font-weight: bold">
                            ...
                            <th width="170">Chủ đề</th>
                            <th width="990">Tin nhắn</th>
                            <th width="150">Ngày đăng</th>
                            <th width="150">Ngày kiểm duyệt</th>
                            <th width="150">Duyệt/Không duyệt </th>
                            <th width="250">Kết quả </th>
                            <th width="70">Xóa </th>
                        </tr>';
            if(isset($_GET['page'])!=""){
            $page = $_GET['page'];
            ...
            $result = oci_parse($conn,"SELECT ID,FULL_NAME,PHONE,EMAIL,SUBJECT,MESSAGE_TXT, TO_CHAR(CREATED_AT,'DD-MM-YYYY HH24:MI:SS'),TO_CHAR(UPDATED_AT,'DD-MM-YYYY HH24:MI:SS'), KIEM_DUYET FROM CONTACT_MESSAGES ORDER BY ID DESC");
            oci_execute($result);
               ...
               if($i>=$startitem){
                $noidung = $row[5]->load();
                    echo "<tr>
                          ...
                          <td>$row[6]</td>
                          <td>$row[7]</td>
                          <td><a href='kiemduyetthu.php?id=".$row[0]."'>Kiểm duyệt</a></td>
                          <td>$row[8]</td>
                          <td><a onClick=\"javascript: return confirm('Bạn có đồng ý xóa thư có mã $row[0]?');\" href='quanlythu.php?page=".$_GET["page"]."&id=".$row[0]."'>Xóa</a></td>
                          </tr>";
            ++$dem;
                    if($dem==8)                           
                      break;
            ...
            echo '</table>';
     if(isset($_GET["id"])!=''){      
    $idthu = $_REQUEST["id"];   //Lấy mã id của thư cần xóa
    $result = oci_parse($conn,"DELETE FROM CONTACT_MESSAGES where ID = $idthu");
    oci_execute($result);
    if(oci_execute($result)){ //Kiểm tra và xuất thông báo
        echo "<p align = 'center'>Đã xóa thư có mã <b>$idthu</b></p>";            
    }
    else echo oci_error($conn); // Nếu không tìm thấy nhận xét thì xuất ra kết nối thất bại
}
            oci_close($conn);
     ?>
    ...
    <div class="row pagination-wrap mt-5">
          <?php 
            include "dbconnect.php";
            $result = oci_parse($conn,"SELECT * FROM CONTACT_MESSAGES");
            oci_execute($result);
            ...
            <div class="custom-pagination ml-auto">
              <?php 
                if(isset($_GET["page"]) !=''){
                  ...
                  $lui=$get-1;
                  echo '<a href="quanlythu.php?page='.$lui.'" class="prev">Trước</a>';
              ?>
              ...
              <?php 
                                ...
                                if($sl_sp>8){
                                    $npage = ceil($sl_sp/8);
                                    for($i = 1;$i<=$npage;++$i){
                                        echo '<a href="quanlythu.php?page='.$i.'">'.$i.'</a>';    
                                    }
                                    
                                }
                                else echo '<a href="quanlythu.php?page=1">1</a>'; 
                             ?>
              </div>
              <?php 
                if(isset($_GET["page"])!=''){
                ...
                $tiep=$get+1;
                echo '<a href="quanlythu.php?page='.$tiep.'" class="next">Tiếp</a>';
              ?>
            </div>
