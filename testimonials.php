...
<div class="site-wrap">
    ...
    <section class="section-hero overlay inner-page bg-image" style="background-image: url('images/hero_1.jpg');" id="home-section">
      <div class="container">
        <div class="row">
          <div class="col-md-7">
            <h1 class="text-white font-weight-bold">Lời chứng thực của ứng viên</h1>
            <div class="custom-breadcrumbs">
              <a href="index.php">Trang chủ</a> <span class="mx-2 slash">/</span>
              <span class="text-white"><strong>Lời chứng thực</strong></span>
            </div>
          </div>
        </div>
      </div>
    ...
    <section class="site-section" id="next-section">
      <div class="container">
        ...
        <div class="row">
          <?php 
            $orc="SELECT FULL_NAME,JOB,MESSAGE_TXT,IMAGE FROM APPLICANT_MESSAGES, USERS WHERE USERS.ID=APPLICANT_MESSAGES.USER_ID ORDER BY APPLICANT_MESSAGES.ID DESC";
                ...
                if($i>0){ 
                  echo '<div class="col-lg-6 mb-4">
                          <div class="block__87154">
                            <blockquote>
                              <p>&ldquo;'.$row[2]->load().'&rdquo;</p>
                            </blockquote>
                            <div class="block__91147 d-flex align-items-center">
                              <figure class="mr-4"><img src="images/'.$row[3].'" alt="Free Website Template by Free-Template.co" class="img-fluid"></figure>
                              <div>
                                <h3>'.$row[0].'</h3>
                                <span class="position">'.$row[1].'</span>
                              </div>
                            </div>
                          </div>
                        </div>';
                  ...
                  if($i==0){
                    echo '<div class="col-lg-12 mb-4">
                            <div class="block__87154 bg-primary">
                              <blockquote>
                                <p class="text-white">&ldquo;'.$row[2]->load().'&rdquo;</p>
                              </blockquote>
                              <div class="block__91147 d-flex align-items-center">
                                <figure class="mr-4"><img src="images/'.$row[3].'" alt="Free Website Template by Free-Template.co" class="img-fluid"></figure>
                                <div>
                                  <h3 class="text-white">'.$row[0].'</h3>
                                  <span class="position position-2">'.$row[1].'</span>
                                </div>
                              </div>
                            </div>
                          </div>';
 ...
 <div class="col-lg-6 mb-5 mb-lg-0">
            <form action="testimonials.php" class="" method="POST">
              ...
              <div class="row form-group">
                ...
                <div class="col-md-6">
                  <label class="text-black" for="lname">Nghể nghiệp</label>
                  <input type="text" id="nghe" name="nghe" class="form-control">
                </div>
              </div>

              <div class="row form-group">              
                <div class="col-md-12">
                  <label class="text-black" for="email">Nội dung: </label> 
                  <textarea type="text" name="noidung" id="noidung" class="form-control" style="width: 300px" value="<?php if(isset($_POST["noidung"])) echo $_POST["noidung"]; else echo ''; ?>"></textarea>
                </div>
              </div>
              <div class="row form-group">
                <div class="col-md-12">
                  <input type="submit" value="Đăng" class="btn btn-primary btn-md text-white" name="Luu">
                </div>
              </div>
            </form>
<?php
include "dbconnect.php";
        ...
        if(isset($_POST["Luu"])) { //Khi nhấn Lưu sẽ thực hiện các câu lệnh            
            $id = test_input($_POST["iduser"]);
            $noidung = test_input($_POST["noidung"]);
            $nghe = test_input($_POST["nghe"]);

            $check = 1;
            if($id==''||$noidung==''||$nghe==''){ //Kiểm tra rỗng nếu không nhập (hoặc thiếu)
                echo "<p align='center'>Vui lòng nhập đủ thông tin!</p>";
                $check = 0;
            }
                 
            $result=oci_parse($conn,"INSERT INTO APPLICANT_MESSAGES (ID,USER_ID,JOB,MESSAGE_TXT,CREATED_AT) VALUES ('','$id','$nghe','$noidung',sysdate)"); //Thêm dữ liệu tương ứng vào database 

            if($check<>0){ //Kiểm tra và xuất thông báo
                oci_execute($result);
                echo "<br><h3 align='center' style='color:red'>Đăng lời đánh giá thành công!</h3>";
            }else echo "<h3 align='center' style='color:red'>Đăng lời đánh giá thất bại! </h3>".oci_error($result);                
        }    
      oci_close($conn);
?>
</div>
