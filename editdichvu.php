...
<div class="site-wrap">
    ...
    <section class="site-section">
      <div class="container">
        ...
        if($flag==true){  
        $id = $_REQUEST["id"];
        $sql = oci_parse($conn,"SELECT SEO_TITLE,SEO_DESCRIPTION FROM CMS"); //Lấy thông tin blog có mã id tương ứng
    oci_execute($sql);
      while($row = oci_fetch_array($sql)){
        ...
        if(isset($_POST["luu"])){  //Khi nhấn Kiểm duyệt sẽ thực hiện các lệnh
          ...
          $result = oci_parse($conn,"UPDATE CMS SET SEO_TITLE = '$tieude', SEO_DESCRIPTION = '$noidung', UPDATED_AT=sysdate  WHERE ID = '$id'"); //Cập nhật dữ liệu tương ứng vào database
          oci_execute($result);
          echo "<p align = 'center'>Cập nhật thành công!!!!</p>";
        }
        echo "<p align = 'center'><a href = 'quanlydichvu.php?page=1'>Quay về </a></p>";
      oci_close($conn);
 ?>
        ...
        <table align="center">
        <tr>
            <td colspan="2"><h2 style="text-align: center; font-family: Arial">Sửa đổi dịch vụ</h2></td>
        </tr>
        <tr>
            <td>Mã dịch vụ:</td>
            <td><label>
              <input type="text" value="<?php echo $id; ?>" readonly=true>
            </label></td>
        </tr>
        <tr>
            <td>Tiêu đề: </td>
        ...
        <tr>
            <td colspan="2" align="center"><input type="submit" name="luu" value="Lưu"></td>
        </tr>
        </table>
    </form>
<?php } ?>
