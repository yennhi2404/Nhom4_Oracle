...
<div class="site-wrap">
    ...
    <section class="site-section">
      <div class="container">
        ...
        if($flag==true){       
        $idhm = $_REQUEST["idhm"];
        $sql = oci_parse($conn,"SELECT JOB_SKILL FROM JOB_SKILLS WHERE ID = '$idhm'"); //Lấy thông tin dự án có mã id tương ứng
      oci_execute($sql);
      while($row = oci_fetch_array($sql))
        ...
        if(isset($_POST["Luu"])){  //Khi nhấn Kiểm duyệt sẽ thực hiện các lệnh
          ...
          $result = oci_parse($conn,"UPDATE JOB_SKILLS SET JOB_SKILL = '$hangmuc' WHERE ID = '$idhm'"); //Cập nhật dữ liệu tương ứng vào database
 ...
 ?>
        ...
        <table align="center">
        <tr>
            <td colspan="2"><h2 style="text-align: center; font-family: Arial">Sửa đổi hạng mục</h2></td>
        </tr>
        <tr>
          <td>Tên hạng mục: </td>
          <td><label>
              <input type="text" name="hangmuc" id="hangmuc" style="width: 300px" value="<?php if(isset($_POST["hangmuc"])) echo $_POST["hangmuc"]; else echo $hangmucc; ?>">
          </label></td>
        </tr>
