...
<div class="site-wrap">
    ...
    <section class="site-section">
      <div class="container">
        ...
        if($flag==true){       
        echo '<h3 align="center" class="style1" style="font-weight:bold">DỊCH VỤ</h3>
                <p align="center">
                    ...
                    <table>
                        <tr style="color:red; font-weight: bold">
                            <th width="90">ID </th>
                            <th width="170">Tên dịch vụ </th>
                            <th width="990">Mô tả </th>
            ...
            $result = oci_parse($conn,"SELECT ID,SEO_TITLE,SEO_DESCRIPTION,TO_CHAR(CREATED_AT,'DD-MM-YYYY HH24:MI:SS'),TO_CHAR(UPDATED_AT,'DD-MM-YYYY HH24:MI:SS') FROM CMS");
            oci_execute($result);
               ...
               if($i>=$startitem){
                 ...
                 $noidung = $row[2]->load();
                      echo "<tr>
                            ...
                            <td>$row[3]</td>
                            <td>$row[4]</td>
                            <td><a href='editdichvu.php?id=".$row[0]."'>Sửa</a></td>
                            <td><a onClick=\"javascript: return confirm('Bạn có đồng ý xóa dịch vụ có mã $row[0]?');\" href='quanlydichvu.php?id=".$row[0]."'>Xóa</a></td>
                            </tr>";                   
                       ++$dem;
                      if($dem==6)                           
                        break;
            ...
            echo '</tr>
                            <tr>
                                <td colspan="10">
                                    <a href="themdichvu.php"><input type="submit" name="themsp" value="Thêm dịch vụ"></a>
                                </td>
                            </tr>
                        </table>';
     ...
     if(isset($_GET["id"])!=''){      
        $id = $_REQUEST["id"];   
        $result = oci_parse($conn,"DELETE FROM CMS where ID = $id");
        oci_execute($result);
        if(oci_execute($result)){ //Kiểm tra và xuất thông báo
            echo "<p align = 'center'>Đã xóa dịch vụ có mã <b>$id</b></p>";            
        }
...
<div class="row pagination-wrap mt-5">
          <?php 
            include "dbconnect.php";
            $result = oci_parse($conn,"SELECT * FROM CMS");
            oci_execute($result);
            ...
            <div class="custom-pagination ml-auto">
              <?php 
                if(isset($_GET["page"]) !=''){
                  ...
                  $lui=$get-1;
                  echo '<a href="quanlydichvu.php?page='.$lui.'" class="prev">Trước</a>';
              ?>
              ...
              <?php 
                                ...
                                if($sl_sp>6){
                                    $npage = ceil($sl_sp/6);
                                    for($i = 1;$i<=$npage;++$i){
                                        echo '<a href="quanlydichvu.php?page='.$i.'">'.$i.'</a>';    
                                    }
                                    
                                }
                                else echo '<a href="quanlydichvu.php?page=1">1</a>'; 
                             ?>
              </div>
              <?php 
                if(isset($_GET["page"])!=''){
                ...
                $tiep=$get+1;
                echo '<a href="quanlydichvu.php?page='.$tiep.'" class="next">Tiếp</a>';
              ?>
            </div>
