...
<div class="site-wrap">
    ...
    <!-- HOME -->
     <section class="section-hero overlay inner-page bg-image" style="background-image: url('images/hero_1.jpg');" id="home-section">
      <div class="container">
        <div class="row">
          <div class="col-md-7">
            <h1 class="text-white font-weight-bold">Chứng thực</h1>
            <div class="custom-breadcrumbs">
              <a href="index.php">Trang chủ</a> <span class="mx-2 slash">/</span>
              <span class="text-white"><strong>Chứng thực</strong></span>
            </div>
          </div>
        </div>
      </div>
    ...
    <section class="site-section">
      <div class="container">
        ...
        if($flag==true){  
        echo '<h3 align="center" class="style1" style="font-weight:bold">CHỨNG THỰC</h3>
                <p align="center">
            ...
            $result = oci_parse($conn,"SELECT APPLICANT_MESSAGES.ID,FULL_NAME,MESSAGE_TXT FROM APPLICANT_MESSAGES, USERS WHERE APPLICANT_MESSAGES.USER_ID = USERS.ID");
            oci_execute($result);
               ...
               if($i>=$startitem){
                 $noidung = $row[2]->load();
                      echo "<tr>
                            ...
                            <td>$row[1]</td>
                            <td>$noidung</td>
                            <td><a onClick=\"javascript: return confirm('Bạn có đồng ý xóa chứng thực có mã $row[0]?');\" href='quanlychungthuc.php?id=".$row[0]."'>Xóa</a></td>
                            </tr>";                   
                       ++$dem;
                      if($dem==6)                           
                        break;
     ...
     if(isset($_GET["id"])!=''){      
        $idct = $_REQUEST["id"];   
        $result = oci_parse($conn,"DELETE FROM APPLICANT_MESSAGES where ID = $idct");
        oci_execute($result);
        if(oci_execute($result)){ //Kiểm tra và xuất thông báo
            echo "<p align = 'center'>Đã xóa chứng thực có mã <b>$idct</b></p>";            
        }
...
<div class="row pagination-wrap mt-5">
          <?php 
            include "dbconnect.php";
            $result = oci_parse($conn,"SELECT * FROM APPLICANT_MESSAGES");
            oci_execute($result);
            ...
            <div class="custom-pagination ml-auto">
              <?php 
                if(isset($_GET["page"]) !=''){
                  ...
                  $lui=$get-1;
                  echo '<a href="quanlychungthuc.php?page='.$lui.'" class="prev">Trước</a>';
              ?>
              ...
              <?php 
                                ...
                                if($sl_sp>6){
                                    $npage = ceil($sl_sp/6);
                                    for($i = 1;$i<=$npage;++$i){
                                        echo '<a href="quanlychungthuc.php?page='.$i.'">'.$i.'</a>';    
                                    }
                                    
                                }
                                else echo '<a href="quanlychungthuc.php?page=1">1</a>'; 
                             ?>
              </div>
              <?php 
                if(isset($_GET["page"])!=''){
                ...
                $tiep=$get+1;
                echo '<a href="quanlychungthuc.php?page='.$tiep.'" class="next">Tiếp</a>';
              ?>
            </div>
            <br>
