...
<div class="site-wrap">
    ...
    <!-- HOME -->
     <section class="section-hero overlay inner-page bg-image" style="background-image: url('images/hero_1.jpg');" id="home-section">
      <div class="container">
        <div class="row">
          <div class="col-md-7">
            <h1 class="text-white font-weight-bold">Khác</h1>
            <div class="custom-breadcrumbs">
              <a href="index.php">Trang chủ</a> <span class="mx-2 slash">/</span>
              <span class="text-white"><strong>Khác</strong></span>
            </div>
          </div>
        </div>
      </div>
    ...
    <section class="site-section">
      <div class="container">
        ...
        if($flag==true){  
        echo '<h3 align="center" class="style1" style="font-weight:bold">QUỐC GIA</h3>
                    <table>
                        <tr style="color:red; font-weight: bold">
                            <th width="170">ID </th>
                            <th width="900">Tên quốc gia </th>
                            <th width="120">Sửa </th>
                            <th width="120">Xóa </th>
                        </tr>';
            if(isset($_GET['page'])!=""){
            ...
            $result = oci_parse($conn,"SELECT ID,COUNTRY FROM COUNTRIES ORDER BY ID ASC");
            oci_execute($result);
            ...
            while($row = oci_fetch_array($result)){
              if($i>=$startitem){
                      echo "<tr>
                            <td>$row[0]</td>
                            <td>$row[1]</td>
                            <td><a href='editquocgia.php?idqg=".$row[0]."'>Sửa</a></td>
                            <td><a onClick=\"javascript: return confirm('Bạn có đồng ý xóa quốc gia có mã $row[0]?');\" href='quanlykhac.php?idqg=".$row[0]."'>Xóa</a></td>
                            </tr>";                   
                       ++$dem;
                      if($dem==20)                           
                        break;
            ...
            echo '</tr>
                            <tr>
                                <td colspan="4">
                                    <a href="themquocgia.php"><input type="submit" name="themqg" value="Thêm quốc gia"></a>
                                </td>
                            </tr>
                        </table>';
     ...
     if(isset($_GET["idqg"])!=''){      
        $idqg = $_REQUEST["idqg"];   
        $result = oci_parse($conn,"DELETE FROM COUNTRIES where ID = $idqg");
        oci_execute($result);
        if(oci_execute($result)){ //Kiểm tra và xuất thông báo
            echo "<p align = 'center'>Đã xóa quốc gia có mã <b>$idqg</b></p>";            
        }
        else echo "Không thể xóa quốc gia này!"; 
    }
            oci_close($conn);
     ?>
<br>
     ...
     include "dbconnect.php";  
        ...
        echo '<h3 align="center" class="style1" style="font-weight:bold">HẠNG MỤC</h3>
                    <table>
                        <tr style="color:red; font-weight: bold">
                            <th width="170">ID </th>
                            <th width="900">Tên hạng mục </th>
                            <th width="120">Sửa </th>
                            <th width="120">Xóa </th>
                        </tr>';
            if(isset($_GET['page'])!=""){
            ...
            $result = oci_parse($conn,"SELECT ID,JOB_SKILL FROM JOB_SKILLS ORDER BY ID ASC");
            oci_execute($result);
            ...
            while($row = oci_fetch_array($result)){
              if($i>=$startitem){
                      echo "<tr>
                            <td>$row[0]</td>
                            <td>$row[1]</td>
                            <td><a href='edithangmuc.php?idhm=".$row[0]."'>Sửa</a></td>
                            <td><a onClick=\"javascript: return confirm('Bạn có đồng ý xóa hạng mục có mã $row[0]?');\" href='quanlykhac.php?idhm=".$row[0]."'>Xóa</a></td>
                            </tr>";                   
                       ++$dem;
                      if($dem==12)                           
                        break;
            ...
            echo '</tr>
                            <tr>
                                <td colspan="4">
                                    <a href="themhangmuc.php"><input type="submit" name="themqg" value="Thêm hạng mục"></a>
                                </td>
                            </tr>
                        </table>';
     ...
     if(isset($_GET["idhm"])!=''){      
        $idhm = $_REQUEST["idhm"];   
        $result = oci_parse($conn,"DELETE FROM JOB_SKILLS where ID = $idhm");
        oci_execute($result);
        if(oci_execute($result)){ //Kiểm tra và xuất thông báo
            echo "<p align = 'center'>Đã xóa hạng mục có mã <b>$idhm</b></p>";            
        }
        else echo "Không thể xóa hạng mục này!"; 
    }
            oci_close($conn);
     ?>
<div class="row pagination-wrap mt-5">
          <?php 
            include "dbconnect.php";
            $result = oci_parse($conn,"SELECT * FROM COUNTRIES");
            oci_execute($result);
            ...
            <div class="custom-pagination ml-auto">
              <?php 
                if(isset($_GET["page"]) !=''){
                  ...
                  $lui=$get-1;
                  echo '<a href="quanlykhac.php?page='.$lui.'" class="prev">Trước</a>';
              ?>
              ...
              <?php 
                                ...
                                if($sl_sp>20){
                                    $npage = ceil($sl_sp/20);
                                    for($i = 1;$i<=$npage;++$i){
                                        echo '<a href="quanlykhac.php?page='.$i.'">'.$i.'</a>';    
                                    }
                                    
                                }
                                else echo '<a href="quanlykhac.php?page=1">1</a>'; 
                             ?>
              </div>
              <?php 
                if(isset($_GET["page"])!=''){
                ...
                $tiep=$get+1;
                echo '<a href="quanlykhac.php?page='.$tiep.'" class="next">Tiếp</a>';
              ?>
            </div>
            <br>
