...
<div class="site-wrap">
    ...
    <section class="section-hero overlay inner-page bg-image" style="background-image: url('images/hero_1.jpg');" id="home-section">
      <div class="container">
        <div class="row">
          <div class="col-md-7">
            <h1 class="text-white font-weight-bold">Thư viện</h1>
            <div class="custom-breadcrumbs">
              <a href="index.php">Trang chủ</a> <span class="mx-2 slash">/</span>
              <span class="text-white"><strong>Thư viện</strong></span>
            </div>
          </div>
        </div>
      </div>
    ...
    <section class="site-section" id="next-section">
      <div class="container">
        <div class="row">
          <?php 
            $orc="SELECT IMAGE FROM PROFILE_PROJECTS";
            $run=oci_parse($conn,$orc);
            oci_execute($run);
             
            while ($row=oci_fetch_array($run)){
              $gallery = explode('*',$row[0],4); 
               for($i=0;$i<=sizeof($row[0],1)+1;$i++){
              echo '<div class="col-md-6 col-lg-4 item">
                      <a href="images/'.$gallery[$i].'" class="item-wrap fancybox" data-fancybox="gallery2">
                        <span class="icon-search2"></span>
                        <img class="img-fluid" src="images/'.$gallery[$i].'">
                      </a>
                    </div>';
              }
            }
