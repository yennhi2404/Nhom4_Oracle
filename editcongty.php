...
<div class="site-wrap">
    ...
    <section class="site-section">
      <div class="container">
        ...
        $sql = oci_parse($conn,"SELECT NAME,WEBSITE FROM COMPANIES WHERE ID = '$idct'"); //Lấy thông tin dự án có mã id tương ứng
        oci_execute($sql);
        ...
        if(isset($_POST["Luu"])){  //Khi nhấn Kiểm duyệt sẽ thực hiện các lệnh
          ...
          $check = 1;
          if($tenct==''||$website==''){ //Kiểm tra rỗng nếu không nhập (hoặc thiếu)
                echo "<p align='center'>Vui lòng nhập đủ thông tin!</p>";
                $check = 0;
          }

          $result = oci_parse($conn,"UPDATE COMPANIES SET NAME = '$tenct', WEBSITE = '$website', UPDATED_AT = sysdate WHERE ID = '$idct'"); //Cập nhật dữ liệu tương ứng vào database
 ...
 ?>
        ...
        <table align="center">
        <tr>
            <td colspan="3"><h2 style="text-align: center; font-family: Arial">Sửa đổi công ty</h2></td>
        </tr>
        <tr>
        <td>Tên công ty: </td>
        <td><label>
    ...
    <tr>
        <td colspan="2"><a href="quanlycongty.php?page=1">Quay về</a></td>
    </tr>
    </table>
    </form>
<?php } ?>
