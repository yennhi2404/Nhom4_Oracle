...
<div class="site-wrap">
    ...
    <section class="site-section">
      <div class="container">
        ...
        if($flag==true){      
        echo '<h3 align="center" class="style1" style="font-weight:bold">DỰ ÁN</h3>
                <p align="center">
                    ...
                    <table>
                        <tr style="color:red; font-weight: bold">
                            <th width="140">ID </th>
                            <th width="90">ID người đăng ký </th>
                            <th width="150">Tên dự án </th>
                            <th width="1900">Mô tả </th>
                            <th width="20">URL</th>
                            <th width="170">Ngày bắt đầu</th>
                            <th width="170">Ngày kết thúc</th>
            ...
            $result = oci_parse($conn,"SELECT ID,USER_ID,NAME,IMAGE,DESCRIPTION,URL,TO_CHAR(DATE_START,'DD-MM-YYYY HH24:MI:SS'),TO_CHAR(DATE_END,'DD-MM-YYYY HH24:MI:SS'),TO_CHAR(CREATED_AT,'DD-MM-YYYY HH24:MI:SS'),TO_CHAR(UPDATED_AT,'DD-MM-YYYY HH24:MI:SS') FROM PROFILE_PROJECTS");
            oci_execute($result);
               ...
               if($i>=$startitem){
                 $noidung = $row[4]->load();
                      echo "<tr>
                            ...
                            <td>$row[8]</td>
                            <td>$row[9]</td>
                            <td><a href='editduan.php?id=".$row[0]."'>Sửa</a></td>
                            <td><a onClick=\"javascript: return confirm('Bạn có đồng ý xóa dự án có mã $row[0]?');\" href='quanlyduan.php?id=".$row[0]."'>Xóa</a></td>
                            </tr>";                   
                       ++$dem;
                      if($dem==6)                           
                        break;
            ...
            echo '</tr>
                            <tr>
                                <td colspan="11">
                                    <a href="themduan.php"><input type="submit" name="themsp" value="Thêm dự án"></a>
                                </td>
                            </tr>
                        </table>';
     ...
     if(isset($_GET["id"])!=''){      
        $idda = $_REQUEST["id"];   
        $result = oci_parse($conn,"DELETE FROM PROFILE_PROJECTS where ID = $idda");
        oci_execute($result);
        if(oci_execute($result)){ //Kiểm tra và xuất thông báo
            echo "<p align = 'center'>Đã xóa dự án có mã <b>$idda</b></p>";            
        }
...
<div class="row pagination-wrap mt-5">
          <?php 
            include "dbconnect.php";
            $result = oci_parse($conn,"SELECT * FROM PROFILE_PROJECTS");
            oci_execute($result);
            ...
            <div class="custom-pagination ml-auto">
              <?php 
                if(isset($_GET["page"]) !=''){
                  ...
                  $lui=$get-1;
                  echo '<a href="quanlyduan.php?page='.$lui.'" class="prev">Trước</a>';
              ?>
              ...
              <?php 
                                ...
                                if($sl_sp>6){
                                    $npage = ceil($sl_sp/6);
                                    for($i = 1;$i<=$npage;++$i){
                                        echo '<a href="quanlyduan.php?page='.$i.'">'.$i.'</a>';    
                                    }
                                    
                                }
                                else echo '<a href="quanlyduan.php?page=1">1</a>'; 
                             ?>
              </div>
              <?php 
                if(isset($_GET["page"])!=''){
                ...
                $tiep=$get+1;
                echo '<a href="quanlyduan.php?page='.$tiep.'" class="next">Tiếp</a>';
              ?>
            </div>
            <br>
