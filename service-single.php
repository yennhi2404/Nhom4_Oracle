...
<div class="site-wrap">
    ...
    <section class="section-hero overlay inner-page bg-image" style="background-image: url('images/hero_1.jpg');" id="home-section">
      <div class="container">
        <div class="row">
          <div class="col-md-7">
            <h1 class="text-white font-weight-bold">Công cụ đơn</h1>
            <div class="custom-breadcrumbs">
              <a href="index.php">Trang chủ</a> <span class="mx-2 slash">/</span>
              <span class="text-white"><strong>Công cụ đơn</strong></span>
            </div>
          </div>
        </div>
      </div>
        ...
        <div class="row">
          <div class="col-lg-3 mr-auto">
            <div class="border p-4 rounded">
              <ul class="list-unstyled block__47528 mb-0">
                <?php 
                  $sql="SELECT ID,SEO_TITLE FROM CMS ORDER BY ID ASC";
                  $result=oci_parse($conn,$sql);
                  oci_execute($result);

                  $dem=1;
                  while ($row=oci_fetch_row($result)){
                    echo '<li><a href="service-single.php?idcc='.$row[0].'&hinh='.$dem.'">'.$row[1]->load().'</a></li>';
                    $dem++;
                    if ($dem==7)
                      $dem=1;
                  }
          ...
          <div class="col-lg-8">
            <?php 
              $ma=$_GET["idcc"];
              $hinh = $_GET["hinh"];
              $sql="SELECT * FROM CMS WHERE ID = '$ma' ";
              $run=oci_parse($conn,$sql);
              oci_execute($run);

              while ($row=oci_fetch_row($run)){
                if($hinh==1)
                    echo '<span class="text-primary d-block mb-5"><span class="icon-magnet display-1"></span></span>';
                  if ($hinh==2) 
                    echo '<span class="text-primary d-block mb-5"><span class="icon-trophy display-1"></span></span>';
                  if ($hinh==4) 
                    echo '<span class="text-primary d-block mb-5"><span class="icon-laptop display-1"></span></span>';
                  if ($hinh==3) 
                    echo '<span class="text-primary d-block mb-5"><span class="icon-search display-1"></span></span>';
                  if ($hinh==5) 
                    echo '<span class="text-primary d-block mb-5"><span class="icon-paper-plane display-1"></span></span>';
                  if ($hinh==6){ 
                    echo '<span class="text-primary d-block mb-5"><span class="icon-plug display-1"></span></span>';
                  }
              $ten = $row[4]->load();
              $mota= $row[5]->load();
                  echo '<h2 class="mb-4">'.$ten.'</h2>';
                  echo '<p>'.$mota.'</p>';
               } 
            ?>
            <p><a href="contact.php" class="btn btn-primary btn-md mt-4">Thuê chúng tôi</a></p>
          </div>
        </div>
      </div>
    </section>
